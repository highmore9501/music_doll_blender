...
def register_tool_scene_props():
    ...
    if not hasattr(bpy.types.Scene, "zheng_string_amplitude"):
        bpy.types.Scene.zheng_string_amplitude = FloatProperty(
            name=T("振幅比例"),
            description=T("弦振动的偏移比例（实际偏移 = 弦长 * 比例）"),
            default=0.005,
            min=0.0001,
            max=0.1,
            precision=4,
...
class ZHENG_OT_tool_create_string_shape_key(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report(
                {'INFO'}, T("生成弦 Shape Key") % scene.zheng_string_index)
            return {'FINISHED'}
        except Exception as e:
            self.report({'ERROR'}, T("生成弦 Shape Key") % str(e))
            return {'CANCELLED'}
...
class ZHENG_OT_tool_create_all_strings_shape_keys(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({'INFO'}, T("生成所有弦 Shape Key"))
            return {'FINISHED'}
        except Exception as e:
            self.report({'ERROR'}, T("生成所有弦 Shape Key") % str(e))
            return {'CANCELLED'}
...
class ZHENG_OT_tool_linear_distribute_recorders(Operator):
    ...
    def execute(self, context):
        try:
            ...
            self.report({'INFO'}, T("线性分布记录器"))
            return {'FINISHED'}
        except Exception as e:
            self.report({'ERROR'}, T("线性分布记录器") % str(e))
            return {'CANCELLED'}
...
def _draw_string_shape_key(layout, scene):
    ...
    col.prop(scene, "zheng_string_index", text=T("弦序号"))
    col = split_row.column()
    col.prop(scene, "zheng_string_amplitude", text=T("振幅"))
    layout.label(text=T("先选中弦（或用弦序号），再执行；需先 Setup 并定位弦记录器"),
                 icon="INFO")


def _draw_linear_distribute(layout, scene):
    """线性分布工具的参数区：使用提示"""
    layout.label(text=T("提示：先选中两个端点记录器（如 s0head / s20head），再执行"),
                 icon="INFO")
    layout.label(text=T("将把该序号范围内的所有记录器线性分布在两端点之间"))
...
def register():
    ...
    bl_label_set(ZHENG_OT_tool_create_string_shape_key, "生成弦 Shape Key")
    bpy.utils.register_class(ZHENG_OT_tool_create_string_shape_key)
    bl_label_set(ZHENG_OT_tool_create_all_strings_shape_keys,
                 "生成所有弦 Shape Key")
    bpy.utils.register_class(ZHENG_OT_tool_create_all_strings_shape_keys)
    bl_label_set(ZHENG_OT_tool_linear_distribute_recorders, "线性分布记录器")
