...
LEFT_HAND_POSITIONS_MAP = {
    'P0': [('Normal', T("Normal"), T("Normal state")),
           ('Outer', T("Outer"), T("Outer state")),
           ('Barre', T("Barre"), T("Barre state"))],
    'P1': [('Normal', T("Normal"), T("Normal state")),
           ('Inner', T("Inner"), T("Inner state")),
           ('Barre', T("Barre"), T("Barre state"))],
    'P2': [('Normal', T("Normal"), T("Normal state")),
           ('Outer', T("Outer"), T("Outer state")),
           ('Barre', T("Barre"), T("Barre state"))],
    'P3': [('Normal', T("Normal"), T("Normal state")),
           ('Inner', T("Inner"), T("Inner state")),
           ('Barre', T("Barre"), T("Barre state"))],
    'P4': [('Normal', T("Normal"), T("Normal state"))]
}
...
class FRET_DANCE_OT_check_status(Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, T("Check complete. See console for details."))
        return {'FINISHED'}
...
class FRET_DANCE_OT_set_state(Operator):
    ...
    def execute(self, context):
        ...
        if right_hand_state is None:
            self.report({'ERROR'}, T("Invalid right hand state"))
            return {'CANCELLED'}
        ...
        self.report({'INFO'}, T("States saved to %s") % target_skeleton.name)
        return {'FINISHED'}
...
class FRET_DANCE_OT_load_state(Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, T("States loaded from %s") %
...
class FRET_DANCE_OT_export_info(Operator):
    ...
    def draw(self, context):
        layout = self.layout
        layout.label(
            text=T("导出将覆盖「人物信息路径」指向的文件内容，确定继续？"),
            icon="ERROR")
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, T("Controller info exported to %s") % filepath)
        return {'FINISHED'}
...
class FRET_DANCE_OT_import_info(Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, T("Controller info imported from %s") % filepath)
        return {'FINISHED'}
...
class FRET_DANCE_OT_select_animation_file(Operator, ImportHelper):
    ...
    def execute(self, context):
        try:
            ...
            if missing_keys:
                self.report(
                    {'ERROR'}, T("Missing keys in JSON file: %s") % ', '.join(missing_keys))
                return {'CANCELLED'}
            ...
            if missing_files:
                self.report(
                    {'WARNING'}, T("Following files not found: %s") % ', '.join(missing_files))

            self.report({'INFO'}, T(
                "Animation config file loaded successfully"))
...
class FRET_DANCE_OT_generate_left_hand_animation(Operator):
    ...
    def execute(self, context):
        ...
        if not config_path or not os.path.exists(config_path):
            self.report(
                {'ERROR'}, T("Please select a valid animation configuration file"))
            return {'CANCELLED'}
        ...
        try:
            ...
            if 'left_hand_animation_file' in config and os.path.exists(config['left_hand_animation_file']):
                ...
                self.report(
                    {'INFO'}, T("Left hand animation generation completed"))
            else:
                self.report(
                    {'WARNING'}, T("Left hand animation file not found or specified"))
                return {'CANCELLED'}
    ...
    def execute(self, context):
        ...
        try:
            ...
            if 'right_hand_animation_file' in config and os.path.exists(config['right_hand_animation_file']):
                ...
                self.report(
                    {'INFO'}, T("Right hand animation generation completed"))
            else:
                self.report(
                    {'WARNING'}, T("Right hand animation file not found or specified"))
                return {'CANCELLED'}
    ...
    def execute(self, context):
        ...
        try:
            ...
            if 'guitar_string_recorder_file' in config and os.path.exists(config['guitar_string_recorder_file']):
                ...
                self.report({'INFO'}, T(
                    "String animation generation completed"))
            else:
                self.report(
                    {'WARNING'}, T("String animation file not found or specified"))
                return {'CANCELLED'}
    ...
    def execute(self, context):
        ...
        try:
            ...
            if 'controller_root_animation_file' in config and os.path.exists(config['controller_root_animation_file']):
                ...
                self.report(
                    {'INFO'}, T("Controller root animation generation completed"))
            else:
                self.report(
                    {'WARNING'}, T("Controller root animation file not found or specified"))
                return {'CANCELLED'}
...
class FRET_DANCE_OT_generate_all_animation(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            if success_count > 0:
                self.report({'INFO'}, T("All animations generation completed"))
                return {'FINISHED'}
            else:
                self.report({'ERROR'}, T(
                    "No animation files found or specified"))
                return {'CANCELLED'}
...
class FRET_DANCE_OT_rename_performer(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({'WARNING'}, T("重命名完成，但重建驱动失败: %s") % str(e))
...
class FRET_DANCE_OT_migrate_legacy(Operator):
    ...
    def execute(self, context):
        ...
        suffix = _get_active_suffix(context)
        if not suffix:
            self.report({'ERROR'}, T("请先输入演奏者后缀"))
            return {'CANCELLED'}
        if performer_utils.has_performer(suffix):
            self.report({'ERROR'}, T("已存在后缀 %s 的演奏者，请换一个后缀") % suffix)
            return {'CANCELLED'}
        skel = _get_active_skeleton(context)
        if skel is None:
            self.report({'ERROR'}, T("请先选中/指定目标骨骼"))
            return {'CANCELLED'}

        base_state = _build_base_state(context, use_skeleton=False)
        if not base_state.migrate_legacy_to_suffix():
            self.report({'ERROR'}, T("迁移失败，详见控制台"))
            return {'CANCELLED'}
        self.report(
            {'INFO'}, T("迁移完成：演奏者 %s (%s)，详见控制台") % (base_state.performer_name, suffix))
        return {'FINISHED'}
...
class FRET_DANCE_PT_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        # 第六大块：保存控制信息
        box = layout.box()
        box.label(text=T("导入/导出人物信息"), icon='EXPORT')
        row = box.row(align=True)
        row.operator("music_doll.fret_dance_import_info", text=T("导入"))
        row.operator("music_doll.fret_dance_export_info", text=T("导出"))
        ...
        row = box.row(align=True)
        row.operator(
            "music_doll.fret_dance_generate_string_animation", text=T("弦动画"))
        row.operator(
            "music_doll.fret_dance_generate_controller_root_animation",
            text=T("吉他偏移"))
        row = box.row()
        row.operator(
            "music_doll.fret_dance_generate_all_animation", text=T("一键生成全部动画"))


def get_left_hand_states_items(self, context):
    """根据Base Position动态返回Left Hand States选项"""
    position = self.fret_dance_base_positions
    return LEFT_HAND_POSITIONS_MAP.get(position, [('Normal', T("Normal"), T("Normal state"))])
...
def get_right_hand_states_items(self, context):
    ...
    items = [
        ...
        ('end', T("End"), T("End position")),
        ('high', T("High"), T("High position")),
    ]
    ...
    if use_vibrato:
        items += [
            ('release', T("Release"), T("Vibrato bar release")),
            ('up', T("Up"), T("Vibrato bar up")),
            ('down', T("Down"), T("Vibrato bar down")),
        ]
    return items
...
def register():
    ...
    bpy.types.Scene.fret_dance_instruments = EnumProperty(
        name=T("Instrument"),
        description=T("Select instrument type"),
        items=[
            ('0', T("Finger Style Guitar"), T("Finger style guitar")),
            ('1', T("Electric Guitar"), T("Electric guitar")),
            ('2', T("Bass"), T("Bass guitar")),
        ],
        default='0'
    )
    ...
    bpy.types.Scene.fret_dance_base_positions = EnumProperty(
        name=T("Position"),
        description=T("Select base position"),
        items=[
            ('P0', T("P0"), T("Position 0")),
            ('P1', T("P1"), T("Position 1")),
            ('P2', T("P2"), T("Position 2")),
            ('P3', T("P3"), T("Position 3")),
            ('P4', T("P4"), T("Position 4"))
        ],
        default='P0'
    )
    ...
    bpy.types.Scene.fret_dance_left_hand_states = EnumProperty(
        name=T("State"),
        description=T("Select left hand state"),
    ...
    bpy.types.Scene.fret_dance_right_hand_states = EnumProperty(
        name=T("State"),
        description=T("Select right hand state"),
        items=get_right_hand_states_items,
        default=0
    )

    bpy.types.Scene.fret_dance_use_vibrato_bar = bpy.props.BoolProperty(
        name=T("Use Vibrato Bar"),
        description=T("Enable vibrato bar (颤音摇杆) for electric guitar"),
        default=False
    )

    bpy.types.Scene.fret_dance_animation_file = StringProperty(
        name=T("Animation Config File"),
        description=T("Path to animation configuration JSON file"),
        subtype='FILE_PATH'
    )

    bpy.types.Scene.fret_dance_string_number = bpy.props.IntProperty(
        name=T("String Number"),
        description=T("The index number of the string"),
        default=0,
        min=0,
        max=6
    )
    ...
    bpy.types.Scene.fret_dance_string_amplitude = bpy.props.FloatProperty(
        name=T("String Amplitude"),
        description=T("振幅与弦长的千分比"),
        default=5.0,
        min=0.1,
        max=50.0,
        precision=4,
    ...
    bpy.types.Scene.fret_dance_target_mesh = PointerProperty(
        name=T("目标Mesh"),
        description=T("存储控制器状态数据的目标角色 Mesh（旧字段，兼容保留）"),
    ...
    bpy.types.Object.fret_dance_controller_data = StringProperty(
        name=T("Controller Data"),
        description=T("JSON: 所有控制器状态数据（左手各位置、右手各位置、指板位置、其他设置）"),
        default="{}",
    )

    # 注册类
    bl_label_set(FRET_DANCE_OT_setup_objects, "设置控制器与指板标记")
    bpy.utils.register_class(FRET_DANCE_OT_setup_objects)
    bl_label_set(FRET_DANCE_OT_check_status, "检查状态")
    bpy.utils.register_class(FRET_DANCE_OT_check_status)
    bl_label_set(FRET_DANCE_OT_set_state, "Set")
    bpy.utils.register_class(FRET_DANCE_OT_set_state)
    bl_label_set(FRET_DANCE_OT_load_state, "Load")
    bpy.utils.register_class(FRET_DANCE_OT_load_state)
    bl_label_set(FRET_DANCE_OT_export_info, "导出人物信息")
    bpy.utils.register_class(FRET_DANCE_OT_export_info)
    bl_label_set(FRET_DANCE_OT_import_info, "导入人物信息")
    bpy.utils.register_class(FRET_DANCE_OT_import_info)
    bl_label_set(FRET_DANCE_PT_main_panel, "FretDance")
    bpy.utils.register_class(FRET_DANCE_PT_main_panel)
    bl_label_set(FRET_DANCE_OT_select_animation_file,
                 "Select Animation Config")
    bpy.utils.register_class(FRET_DANCE_OT_select_animation_file)
    bl_label_set(FRET_DANCE_OT_generate_left_hand_animation,
                 "Generate Left Hand Animation")
    bpy.utils.register_class(FRET_DANCE_OT_generate_left_hand_animation)
    bl_label_set(FRET_DANCE_OT_generate_right_hand_animation,
                 "Generate Right Hand Animation")
    bpy.utils.register_class(FRET_DANCE_OT_generate_right_hand_animation)
    bl_label_set(FRET_DANCE_OT_generate_string_animation,
                 "Generate String Animation")
    bpy.utils.register_class(FRET_DANCE_OT_generate_string_animation)
    bl_label_set(FRET_DANCE_OT_generate_controller_root_animation,
                 "Generate Controller Root Animation")
    bpy.utils.register_class(FRET_DANCE_OT_generate_controller_root_animation)
    bl_label_set(FRET_DANCE_OT_generate_all_animation,
                 "Generate All Animations")
    bpy.utils.register_class(FRET_DANCE_OT_generate_all_animation)
    bl_label_set(FRET_DANCE_OT_duplicate_performer, "复制角色")
    bpy.utils.register_class(FRET_DANCE_OT_duplicate_performer)
    bl_label_set(FRET_DANCE_OT_rename_performer, "重命名当前角色")
    bpy.utils.register_class(FRET_DANCE_OT_rename_performer)
    bl_label_set(FRET_DANCE_OT_migrate_legacy, "迁移旧场景到当前演奏者")
    ...
    ui_utils.register_instrument(
        "fret_dance", T("FretDance 吉他"), FRET_DANCE_PT_main_panel,
